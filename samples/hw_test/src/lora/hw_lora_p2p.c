...
LOG_MODULE_REGISTER(hw_lora_p2p, LOG_LEVEL_INF);
...
static struct k_work_delayable cw_done_work;
...
{
	ARG_UNUSED(work);
	hw_at_resp_line("%s", evt_line);
}

static void cw_done_work_handler(struct k_work *work)
{
	ARG_UNUSED(work);
	ensure_lock();
...
void hw_lora_p2p_init(void)
{
	...
	k_work_init_delayable(&cw_done_work, cw_done_work_handler);
...
bool hw_lora_p2p_is_busy(void)
{
	...
	ensure_lock();
	k_mutex_lock(&p2p_lock, K_FOREVER);
	busy = tx_in_progress || cw_in_progress || async_rx_active || rx_thread_running ||
...
bool hw_lora_p2p_is_active(void)
{
	...
	ensure_lock();
	k_mutex_lock(&p2p_lock, K_FOREVER);
	active = (rx_mode != P2P_RX_OFF) || tx_in_progress || cw_in_progress;
	k_mutex_unlock(&p2p_lock);
	return active;
}
...
int hw_lora_p2p_params_set(const char *param)
{
	...
	ensure_lock();
	k_mutex_lock(&p2p_lock, K_FOREVER);
	if (tx_in_progress || cw_in_progress ||
...
int hw_lora_p2p_send_payload(const uint8_t *data, size_t len)
{
	...
	if (tx_in_progress || cw_in_progress || (rx_mode == P2P_RX_CONT_LOCKED) ||
...
int hw_lora_cw_format(char *out, size_t out_len)
{
	...
	ensure_lock();
	k_mutex_lock(&p2p_lock, K_FOREVER);
	n = snprintk(out, out_len, "%u:%d:%u", cw.freq_hz, (int)cw.tx_dbm, cw.time_ms);
	k_mutex_unlock(&p2p_lock);
...
int hw_lora_cw_start(const char *param)
{
	...
	memcpy(buf, param, strlen(param) + 1U);
	if (!split_colon_fields(buf, parts, 3U)) {
		return -EINVAL;
	}
	...
	errno = 0;
	power = strtol(parts[1], &end, 10);
	if ((errno != 0) || (*end != '\0') || (power < 5) || (power > 22)) {
		return -EINVAL;
	}

	errno = 0;
	time_ms = strtoul(parts[2], &end, 10);
	if ((errno != 0) || (*end != '\0') || (time_ms > 65535UL)) {
		return -EINVAL;
	}

	/* Zephyr/SX126x CW API takes duration in seconds. time_ms=0 => long-running. */
	if (time_ms == 0UL) {
		duration_s = 65535U;
	} else {
		duration_s = (uint16_t)((time_ms + 999UL) / 1000UL);
		if (duration_s == 0U) {
			duration_s = 1U;
		}
	}
	...
	ensure_lock();
	k_mutex_lock(&p2p_lock, K_FOREVER);
	if (tx_in_progress || cw_in_progress || (rx_mode != P2P_RX_OFF)) {
		k_mutex_unlock(&p2p_lock);
		return -EBUSY;
	}

	cw.freq_hz = (uint32_t)freq;
	cw.tx_dbm = (int8_t)power;
	cw.time_ms = (uint16_t)time_ms;
	cw_in_progress = true;
	k_mutex_unlock(&p2p_lock);

	ret = lora_test_cw(lora_dev, (uint32_t)freq, (int8_t)power, duration_s);
	if (ret < 0) {
		k_mutex_lock(&p2p_lock, K_FOREVER);
		cw_in_progress = false;
		k_mutex_unlock(&p2p_lock);
	...
	/* Driver frees the modem on CW TxTimeout (duration_s seconds). */
	(void)k_work_schedule(&cw_done_work, K_SECONDS(duration_s));
	return 0;
}
